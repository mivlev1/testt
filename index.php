...
try{
    ...
    $power= implode(",",$_POST['superpower']);

    $username = 'u41731';
    $pass = '7439940';
    //Представляет собой соединение между PHP и сервером базы данных.
    $conn = new PDO("mysql:host=localhost;dbname=u41731", username,pass,  array(PDO::ATTR_PERSISTENT => true));

    //Подготавливает инструкцию к выполнению и возвращает объект инструкции
    $user = $conn->prepare("INSERT INTO application SET name = ?, email = ?, dob = ?, gender = ?, limbs = ?, bio = ?, che = ?");
    ...
    $user1 = $conn->prepare("INSERT INTO power SET id = ?, power = ?");
    $user1 -> execute([$id_user, $power]);
    $result = true;
